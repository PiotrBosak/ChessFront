...
updateCheckOrMate (Game game) =
  if game.gameStatus == Checking then
    let
      kingMate = isKingMated (wrap game) (if game.turn == WhiteTurn then WhitePiece else BlackPiece)
...
move from to (Game game) =
  do
    ...
    pure $ Game
      { previousBoard: Just currentBoard
...
castlingMove (Move move) (Game game) =
  do
    ...
    pure
      ( ( updateBoard tileAfterKing (wrap move)
            >>> updateBoard kingNewTile (wrap move)
            >>> updateBoard (wrap oldRookTile) (wrap move)
            >>> updateBoard newRookTile (wrap move)
        ) currentBoard
      )
...
updateBoard tile move board =
  let
    afterFirst = M.alter (\_ -> Just tile) (position tile) board.tiles
  in
    board { tiles = afterFirst, previousMove = Just move }
...
cannotBeDefended (Game game) color =
  ...
  in
    all (\(Game game) -> isKingChecked game.currentBoard color) allScenarios
